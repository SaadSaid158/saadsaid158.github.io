...
function StarButtonInner({
  ...
  return (
    <>
      <div
        className="animate-star-btn absolute aspect-square bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)]"
        style={{ ...style, width: `var(--light-width)` } as CSSProperties}
      />
      <div
...
export function StarButton({
  ...
  children,
  ...buttonProps
}: StarButtonProps & Omit<React.ComponentProps<"button">, "children">) {
  const sharedStyle = {
    "--duration": String(duration ?? 3),
    "--light-width": `${lightWidth ?? 110}px`,
    "--light-color": lightColor ?? "#FAFAFA",
  ...
  const innerProps: StarButtonProps = { children, lightWidth, duration, lightColor, backgroundColor, borderWidth };
  ...
  if (href) {
    return (
      ...
      >
        <StarButtonInner {...innerProps}>{children}</StarButtonInner>
      </a>
    );
  }

  return (
    <button style={sharedStyle} className={sharedClass} {...buttonProps}>
      <StarButtonInner {...innerProps}>{children}</StarButtonInner>
    </button>
  );
}
